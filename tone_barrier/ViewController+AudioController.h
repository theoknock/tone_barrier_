...
//    randomizer musical_note_random_generator = random_generator(normalized_random_generator())(^ double (double n) { return n; })(^ double (double n) { return (scale(n, MusicalNoteA, MusicalNoteAFlat, 0.0, 1.0)); });
...
//        __block Float32 theta_increment_t = (M_PI_SQR * musical_note_random_generator() / sampleRate);
//        return ^ (AVAudioFrameCount frame) {
//            if ((frame = frame % 50) == split_frame) (theta_increment_t = (M_PI_SQR * musical_note_random_generator() / sampleRate));
...
//        __block Float32 theta_increment_t = (M_PI_SQR * musical_note_random_generator() / sampleRate); // tonal(note()) is wrong; use root_frequency generated by note() on the other channel
//        return ^ (AVAudioFrameCount frame) {
//            if ((frame = frame % 50) == split_frame) (theta_increment_t = (M_PI_SQR * musical_note_random_generator() / sampleRate));
//            static Float32 theta;
//            !((theta += theta_increment_t) > M_PI_SQR) ?: (theta -= M_PI_SQR);
...
/*
 ...
 AVAudioPlayerNode * player_node = (AVAudioPlayerNode *)retained_object();
     ...
     [_audioEngine connect:player_node My
 :_mixerNode format:_audioFormat]
 AVAudioPlayerNode * player_node = (AVAudioPlayerNode *)retained_object();
...
static AVAudioEngine const * (^audio_engine)(AVAudioPlayerNode * _Nullable) = ^(AVAudioPlayerNode * _Nullable player_node) {
    static dispatch_once_t onceSecurePredicate;
    dispatch_once(&onceSecurePredicate, ^{
